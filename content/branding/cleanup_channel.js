...
async function updateChannel(token, channelId) {
  return new Promise((resolve, reject) => {
    const updateData = {
      id: channelId,
      brandingSettings: {
...
🏢 Working with healthcare, finance, and tech companies across the Pacific Northwest

📧 Free 15-min assessment: https://storvita.com
📅 Book a call: https://calendly.com/chris-iorg

New videos weekly on network security, infrastructure design, and IT leadership.`,
          keywords: 'CCIE Security, Cisco, Network Security, Enterprise Networking, SD-WAN, Zero Trust, Cybersecurity, IT Infrastructure, Network Engineer, Palo Alto, Firewall, CCIE',
          country: 'US',
          defaultLanguage: 'en'
        }
      }
...
async function main() {
  ...
  console.log('Current title:', channel.brandingSettings?.channel?.title);
  
  console.log('\nUpdating channel branding (removing old references)...');
  await updateChannel(token, channel.id);
  
  console.log('\n🧹 CLEANUP COMPLETE!');
  console.log('Channel: https://youtube.com/channel/' + channel.id);
}
